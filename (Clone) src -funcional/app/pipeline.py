...
import matplotlib                 # importa después de fijar la variable, por seguridad
matplotlib.use("Agg")
...
from .m04_data_split.application import service as split_srv
...
RAW_PARTITIONED_DIR.mkdir(parents=True, exist_ok=True)
# -------------------------------------------------------------------------
...
def run() -> None:
    ...
    ruta_parquet = RAW_DIR / "df_raw.parquet"
    ...
    ruta_parquet2 = RAW_PARTITIONED_DIR/ "train_df.parquet"
    ruta_parquet3 = RAW_PARTITIONED_DIR/ "test_df.parquet"
    ruta_parquet4 = RAW_PARTITIONED_DIR/ "backtest_df.parquet"
    df.to_parquet(ruta_parquet2)
    df.to_parquet(ruta_parquet3)
    df.to_parquet(ruta_parquet4)
    ...
    automl = FLAMLWrapper(task="regression",  #NUEVO
                      time_budget=300,
                      metric="mae")         
