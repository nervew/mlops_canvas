...
LATENCY = Gauge("prediction_latency_seconds", "Prediction latency", registry=REGISTRY)
ERRORS = Gauge("prediction_errors", "Prediction errors", registry=REGISTRY)
...
async def predict(payload: PredictionRequest, service: InferenceService = Depends(get_service)) -> PredictionResponse:
    try:
        with LATENCY.time():
