...
def json_safe(o: Any) -> Any:
    ...
    - pandas.Timestamp / datetime / date / time -> ISO-8601 (.isoformat)
    - numpy.datetime64 -> string (np.datetime_as_string)
    - pandas.Timedelta -> .isoformat() (o str)
    - numpy/pandas scalars -> .item()
    - numpy.ndarray / set / tuple -> list
    - dict/list -> se recorre recursivamente
    """
    # Base ya serializable
    ...
    # pandas Timestamp (ISO-8601)
    if isinstance(o, pd.Timestamp):
        if pd.isna(o):
            return None
        return o.isoformat()  # ISO 8601 recomendado para datetimes :contentReference[oaicite:2]{index=2}

    # datetime/date/time de Python
    if isinstance(o, (datetime, date, time)):
        return o.isoformat()  # ISO-8601 :contentReference[oaicite:3]{index=3}

    # numpy.datetime64
    if isinstance(o, np.datetime64):
        try:
            return np.datetime_as_string(o, unit="us")  # ISO-like string :contentReference[oaicite:4]{index=4}
        except Exception:
            return str(o)

    # pandas.Timedelta / Period / Interval
...
def _to_builtin(x: Any) -> Any:
    """Convierte scalars/fechas a tipos JSON-serializables (wrapper simple)."""
    return json_safe(x)
