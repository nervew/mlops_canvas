...
# Ruta absoluta del requirements.txt
REQUIREMENTS_PATH = "/Workspace/Users/jorgee.lopez@adres.gov.co/mlops_canvas/requirements.txt"
...
def run():
    ...
    # 2. EDA univariado (sobre los datos brutos)
    eda_report = eda_uni.run(df)

    # --- Conversión robusta para estadística básica ---------
    desc = eda_report.description
    if isinstance(desc, dict):
        import pandas as pd
        desc = pd.DataFrame(desc).T
    print("Estadísticas básicas (head):\n", desc.head(), "\n")
    # -------------------------------------------------------
